...
import joblib
app = Flask("DiabetesCureApp")
model = joblib.load("Random_Forest_Diabetes.h5")
@app.route("/diabetes")
def home():
...
def dia():
    ...
    #  x9 = request.args.get("z9")
      ...
      output = model.predict([[ int(x1), int(x2), int(x3), int(x4), float(x5), int(x6), int(x7), int(x8)]])
      if str(round(output[0])) == 1:
         data="Diabetic Detected !!"
